...
// T_const symbol is defined in ldm.cuh and should be visible

// ---- CRAM48 constants (unchanged names/values) ----
static const double ALPHA0_48 = 2.258038182743983e-47;
static const double alpha_re_48[24] = {
...
static const double alpha_im_48[24] = {
...
static const double theta_re_48[24] = {
...
static const double theta_im_48[24] = {
...
// (남겨둠) CSV를 float로 읽는 헬퍼 — 기존에 있던 함수 유지
inline bool load_A_matrix(const char* filename, float* A_matrix) {
    ...
    for (int i = 0; i < N_NUCLIDES; ++i) {
        ...
        for (int j = 0; j < N_NUCLIDES; ++j) {
            if (std::getline(ss, cell, ',')) A_matrix[i * N_NUCLIDES + j] = cell.empty() ? 0.0f : std::stof(cell);
            else A_matrix[i * N_NUCLIDES + j] = 0.0f;
        }
    }
    return true;
}
...
void LDM::gauss_solve_inplace(std::vector<double>& M, std::vector<double>& b, int n) {
    for(int k=0;k<n;k++){
        int piv = k; double pmax = std::fabs(M[k*n + k]);
        for(int i=k+1;i<n;i++){
...
// ====== 변경된 수식 반영: α0는 마지막에 곱하고, 각 시프트의 RHS는 현재 result ======
                                    ...
                                    std::vector<double>& col_out)
{
    ...
    // dt는 외부에서 정의된 시뮬레이션 설정을 사용 (기존과 동일한 변수 사용)
    ...
    // 각 극점에 대해: (B - θ_k I) y = result 를 풀고 result += 2 Re(α_k y)
    for(int k=0;k<24;k++){
        double tr = theta_re_48[k], ti = theta_im_48[k];
        double ar = alpha_re_48[k], ai = alpha_im_48[k];

        // 2n x 2n 실수 블록 행렬 M 구성
        std::vector<double> M(dim*dim, 0.0);
        for(int r=0;r<n;r++){
            for(int c=0;c<n;c++){
                double Bij = B[r*n + c];
                M[r*dim + c]               = Bij; // Re block
                M[(r+n)*dim + (c+n)]       = Bij; // Re block (복제)
            }
            M[r*dim + r]           -= tr;  // -theta_re on diagonal (top-left)
            M[(r+n)*dim + (r+n)]   -= tr;  // -theta_re on diagonal (bottom-right)
            M[r*dim + (r+n)]        =  ti; // +theta_im (top-right)
            M[(r+n)*dim + r]        = -ti; // -theta_im (bottom-left)
        }
...
bool LDM::build_T_matrix_and_upload(const char* A60_csv_path) {
    ...
    for(int j=0;j<N_NUCLIDES;j++){
        // e_j에 대한 exp(dt*A) * e_j 계산 → 결과가 T의 j번째 컬럼
...
// (유지) 디바이스 측 벡터-행렬 곱: n <- T * n
__device__ __forceinline__
void cram_decay_calculation(const float* __restrict__ T_matrix, float* __restrict__ concentrations)
{
    float x[N_NUCLIDES];
    #pragma unroll
