...
public:
    MemoryDoctor() : enabled(false), log_dir("logs/memory_doctor/") {}
    ...
    // Clean all .txt files in the log directory
    ...
    // Log data being sent from LDM to Python (with explicit iteration number)
    void logSentData(const std::string& data_type, const float* data,
                     int rows, int cols, int iteration = -1, const std::string& extra_info = "") {
        if (!enabled) return;
